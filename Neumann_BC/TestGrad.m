% Test the accuracy of the gradient reconstruction
...
for mesh_i = 1:length(mesh_names)
    ...
    for K = 0:MAX_K    
        ...
        for mesh_j = 1:length(meshes{mesh_i})
            ...
            error{mesh_i,K+1}(mesh_j) = sqrt(HHOIntegrateGeneral(hho, diff))/sqrt(HHOIntegrateGeneral(hho,norm_gradu));
